...
def windows_over_thresh(data, thresh, keys, spec):
    '''
...
def main(species, outPath, base_path, name, threshold, samples):
    samples = f_utils.set_path_to_files_glob(samples, 'TAB_3p_summary_yRNA.txt')
    datout, window, tot_c = get_data_from_file(samples, species)

    all_wind = windows_to_norm_counts(datout, window, tot_c)
    out_name = write_output(all_wind, '{}_all.csv'.format(name), outPath)
    if threshold:
        over_thresh = windows_over_thresh(all_wind, threshold, window, species)
        out_name = write_output(over_thresh, '{}_over_{}.csv'.format(name, threshold), outPath)
...
if __name__ == '__main__': 
    ...
    parser.add_argument(
             'name', 
             action='store', 
             help='Prefix name for the file (eg miR, RPMYM, ect)')
    parser.add_argument(
             'samples', 
             action='store', 
             nargs='+',
    ...
    parser.add_argument(
            ...
            actions = 'store',
            default = None,
            help = 'Threshold which must be exceeded to be included')
    arg = parser.parse_args()
    main(arg.sp, arg.outPath, arg.base_path, arg.name, arg.threshold, arg.samples)
